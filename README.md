...
- Style variants are expressed through background tone, point size scaling, and opacity ranges:
  - `minimal`: stark black on white with high contrast.
  - `stones`: warm off-white ground with heavier, stone-like marks.
  - `wabi_sabi`: muted tones with smaller, understated points.
  - `sumi`: ink-like strokes with wider opacity variation.
  - `ukiyo`: flatter, bolder marks on a print-like background.
  - `shizen`: soft gray background with organic sizing.
  - `seijaku`: very subtle marks to emphasize stillness.
  - `kanso`: simplest, uncluttered black on white.
  - `yugen`: faint marks and low contrast for depth.
  - `mono_no_aware`: gentle contrast for an ephemeral feel.
