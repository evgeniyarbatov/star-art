...
def _place_labels(ax, stars, color="#1f1f1f"):
    fig = ax.figure
    fig.canvas.draw()
...
# ---------------------------
# NEW: approximate “shortest path visiting all stars” (TSP-ish)
# Nearest-neighbor + 2-opt improvement. Works well for small N.
# ---------------------------
...
def compute_shortest_visit_order(x, y, mag=None):
    """
    Returns an order of indices that forms a short open path visiting all points once.
    Heuristic:
      - start at brightest star (min magnitude) if mag is provided, else start at 0
    ...
    # 2-opt on open path (segment reversals still help)
    order = _two_opt(points, order, max_passes=7)
    return order


# ---------------------------
# UPDATED: different Japanese-inspired minimalist style + background
# and draw the travel path line through stars
# ---------------------------
def wabi_sabi_minimal_style(stars):
    if stars is None or stars.get("count", 0) == 0:
        return None, "white"

    # Paper-like warm background (different from your original)
    bg = "#f2efe6"       # washi-beige
    ink = "#141414"      # sumi-ink near-black
    path_ink = "#2a2a2a" # slightly lighter for subtle lines
    label_ink = "#1f1f1f"
    ...
    _place_labels(ax, stars, color=label_ink)

    ax.set_xlim(-r_max, r_max)
    ax.set_ylim(-r_max, r_max)
    ax.axis("off")

    # Very subtle boundary circle (more minimalist than before)
