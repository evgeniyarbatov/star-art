...
def add_info_text(fig, location, obs_time, count, fov, azimuth, altitude, bg_color):
    ...
    info_text = (
        ...
        f"{time_str}  |  "
        f"Galaxies {count}  |  FOV {fov}°  |  Az {azimuth}°  Alt {altitude}°"
    )
...
def get_galaxies(observer, obs_time, center_alt, center_az, fov):
    alt_list = []
    az_list = []
    mags = []
    names = []
...
def create_artwork(location, fov, azimuth, altitude):
    ...
    objects = get_galaxies(observer, obs_time, altitude, azimuth, fov)
    if objects is None or objects.get("count", 0) == 0:
